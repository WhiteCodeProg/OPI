...
int main() {
    ...
    int maxWeight; // максимальный вес для аттракциона
    int* randomWeight = new int[maxWeight]; // массив желающих прокатиться на аттракционе
    ...
    for (int i = 0; i < maxWeight; i++) {
        randomWeight[i] = rand() % 50 + 50; // генерация случайного веса для каждого человека в диапазоне от 50 до 100 кг
        sum += randomWeight[i];//суммирование веса желающих
        count++;
        if (sum > maxWeight) {//если сумма превышает максимальный вес
            sum = sum - randomWeight[i];//вычитание привышенного веса
            count--;
            break;
        }
    }
